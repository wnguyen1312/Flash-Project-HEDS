...
subroutine Simulation_initBlock(blockId)
  ...
  use Simulation_data, ONLY : sim_gCell,sim_beta, sim_B0, sim_y0, sim_ca, sim_vy0, sim_sigma, sim_theta,&
  ...
  do k = blkLimitsGC(LOW,KAXIS),blkLimitsGC(HIGH,KAXIS)
     do j = blkLimitsGC(LOW,JAXIS),blkLimitsGC(HIGH,JAXIS)
        do i = blkLimitsGC(LOW,IAXIS),blkLimitsGC(HIGH,IAXIS)
           ...
           solnData(VELX_VAR,i,j,k)= 0.5 * tanh(yCoord(j)/sim_y0)
           solnData(VELY_VAR,i,j,k)= sim_vy0 * sin(2*3.14*xCoord(i)) * exp(-yCoord(j)**2/sim_sigma**2)
           solnData(VELZ_VAR,i,j,k)= 0.
           solnData(PRES_VAR,i,j,k)=.5*sim_beta*sim_B0**2
 
           solnData(MAGX_VAR,i,j,k)= sim_B0 * cos(sim_theta)
           solnData(MAGY_VAR,i,j,k) = 0.
           solnData(MAGZ_VAR,i,j,k)= sim_B0 * sin(sim_theta)
...
#if NFACE_VARS > 0
              if (sim_killdivb) then
                 facexData(MAG_FACE_VAR,i,j,k) = sim_B0 * cos(sim_theta)
